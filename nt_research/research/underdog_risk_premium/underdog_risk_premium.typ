...
#show: ieee.with(
  ...
  abstract: [
    We document systematic mispricing in Kalshi's college football prediction markets during the 2025 season. High-probability contracts (90-99 cents) are consistently underpriced by approximately 2.73 percentage points in the hours preceding kickoff. We attribute this inefficiency to retail traders' preference for lottery-like payoffs on longshot bets. A trading strategy exploiting this pattern achieves a 98.58% win rate (139, 141 trades), though concentration risk from expensive contracts limits risk-adjusted returns to a Sharpe ratio of 0.19 (unannualized). Our findings suggest that while market inefficiencies exist and are statistically significant, their exploitation requires careful risk management due to severe tail risk exposure.
  ],
  authors: (
    (
      name: "Andrew Hall",
...
Our calibration analysis reveals significant mispricing in the extreme price bins. Contracts in the $(0, 10]$ bin show a delta of $-3.74$ percentage points, while those in the $(90, 99]$ bin exhibit a delta of $+2.73$ percentage points, both statistically significant. This indicates that low-probability contracts are systematically overpriced, while high-probability contracts are underpriced relative to their true win rates.
...
Implementing this strategy generates 141 trades over our sample period, with 139 winners and only 2 loss. However, the concentration in high-probability contracts creates asymmetric risk exposure. While individual gains are modest, potential losses on any single trade are substantial. The winning trades alone achieve a Sharpe ratio of 1.3 (unannualized), but incorporating the single losing trade reduces the overall strategy Sharpe ratio to 0.19 (unannualized).
